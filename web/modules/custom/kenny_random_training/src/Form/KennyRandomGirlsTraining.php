...
class KennyRandomGirlsTraining extends FormBase {
  ...
  public function getFormId() {
    return 'kenny_random_girls_training_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $muscle_groups = $form_state->get('muscle_groups');
    if ($muscle_groups === NULL) {
      $muscle_groups = $form_state->set('muscle_groups', $this->t('- Select a muscle groups -'));
      $muscle_groups = $this->t('- Select a muscle groups -');
    }
    $form['muscle_groups'] = [
      '#type' => 'select',
    ...
    if ($num_exercises === NULL) {
      $name_field = $form_state->set('num_exercises', 5);
      $num_exercises = 5;
    }
    ...
    $form['exercise_selection'] = [
      ...
      '#ajax' => [
        'callback' => '::chooseExerciseCallback',
        'wrapper' => 'exercise-selection',
      ],
    ];
...
//      '3' => 177,
//    ];
//    $keys_options = array_keys($exercises_options);
//    $approve_array = array_diff($keys_options, array_values($excluded_exercise));
//    dump($approve_array);
    return $form;
   ...
   * @return array
   */
  public function chooseExerciseCallback(array &$form, FormStateInterface $form_state) {
  ...
  public function createExerciseSelectField(&$form, $form_state, $index) {
    ...
    if (!empty($form_state->getValue('muscle_groups'))) {
      ...
      if (!$index == 0) {
        ...
        $random_value = array_rand($exercises_options);
      } else {
        $random_value = $this->config('kenny_girls_stats.settings')->get('quadriceps');
      }
